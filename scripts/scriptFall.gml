...
if(place_meeting(x, (y + vspeed), oBlockParent) && hspeed == 0){    //if about to hit ground and still
    state = state.stand;
}else if(place_meeting(x, (y + vspeed), oBlockParent)){ //if about to hit ground and moving
    state = state.run;
}
